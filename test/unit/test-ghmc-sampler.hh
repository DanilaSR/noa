...
inline PhaseSpaceFoliationOpt get_hamiltonian(
    ...
    torch::manual_seed(utils::SEED);
    return hamiltonian(log_funnel, conf_funnel)(theta_.to(device, false, true),
                                                momentum_.to(device, false, true));
}
...
inline HamiltonianFlow get_hamiltonian_flow(
        ...
        torch::DeviceType device) {
    torch::manual_seed(utils::SEED);
    return hamiltonian_flow(log_funnel, conf_funnel)(theta_.to(device, false, true),
                                                     momentum_.to(device, false, true));
}
...
inline void test_hamiltonian_flow(torch::DeviceType device = torch::kCPU) {
    ...
    ASSERT_TRUE(theta_flow.at(1).device().type() == device);
    ASSERT_TRUE(momentum_flow.at(1).device().type() == device);
    ASSERT_TRUE(energy.at(1).device().type() == device);

    const auto theta_proposal = theta_flow.at(1).to(torch::kCPU, false, true);
    auto err = (theta_proposal - GHMCData::get_expected_flow_theta()).abs().sum().item<float>();
    ASSERT_NEAR(err, 0., 1e-3);

    const auto momentum_proposal = momentum_flow.at(1).to(torch::kCPU, false, true);
